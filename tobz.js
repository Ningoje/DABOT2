...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6285747852482@c.us'
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isAdmin && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    tobz.reply(from, `*「 GRRUP LINK TERDETEKSI 」*\nTerdeteksi mengirimkan link grup chat, maaf anda dikick dari grup!`, id).then(() => {
        ...
        if (chats == 'tarik sis'){
    tobz.sendPtt(from, './media/tariksis.mp3', id)
      }
      if (chats == 'Tarik sis'){
    tobz.sendPtt(from, './media/tariksis.mp3', id)
      }   
    ...
    tobz.sendPtt(from, './media/tariksis.mp3', id)
      ...
      if (chats == 'kangcopet'){
        tobz.sendPtt(from, './media/welot.mp3', id)
      }
      if (chats == 'welotka'){
        tobz.sendPtt(from, './media/welot.mp3', id)
      }     
      if (chats == 'iri?'){
        tobz.sendPtt(from, './media/bilangbos.mp3', id)
      }
      if (chats == 'Iri?'){
        tobz.sendPtt(from, './media/bilangbos.mp3', id)
      }
      ...
      if (chats == 'desah'){
        tobz.sendPtt(from, './media/desah.mp3', id)
      }
      if (chats == 'Desah'){
        tobz.sendPtt(from, './media/desah.mp3', id)
      }
      if (chats == 'Dahyun'){
        tobz.sendPtt(from, './media/dahyun.mp3', id)
      }
      if (chats == 'dahyun'){
        tobz.sendPtt(from, './media/dahyun.mp3', id)
      }
      if (chats == 'Dek posting'){
        tobz.sendPtt(from, './media/cringe.mp3', id)
      }
      if (chats == 'dek posting'){
        tobz.sendPtt(from, './media/cringe.mp3', id)
      }
        ...
        function isReg(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Anda belum tedaftar didatabase\nUntuk mendaftar kirim ${prefix}daftar /nama/umur\n\nContoh format: ${prefix}daftar /Ambrose/20\n\nCukup dengan menggunakan nama depan/panggilan saja`, id) //if user is not registered
            }
        }
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Anda belum cukup umur untuk menggunakan bot\n\nAnda bisa mendaftar ulang lewat owner dengan cara chat owner\nUntuk mendapatkan nomor owner ketik ${prefix}owner`, id) //if user is not registered
            }
        }
	...
	const sotoy = [
        ...
        const mess = {
            wait: 'Sedang diproses... Silahkan tunggu sebentar!',
            magernulissatu: 'Sedang diproses... Silahkan tunggu sebentar!',
            error: {
                St: 'Kirim gambar dengan caption .sticker, atau reply gambar yang sudah dikirim!',
                Ti: 'Kirim gambar dengan caption .toimg, atau reply gambar yang sudah dikirim!',
                Qm: 'Terjadi kesalahan!',
                Yt3: '[❗] Terjadi kesalahan, tidak dapat meng konversi ke mp3!',
                Yt4: '[❗] Terjadi kesalahan, mungkin error di sebabkan oleh sistem.',
                Ig: '[❗] Terjadi kesalahan, mungkin karena akunnya private',
                Ki: 'Maaf, bot tidak dapat mengeluarkan admin grup!',
                Sp: 'Maaf, bot tidak dapat mengeluarkan member *premium*!',
                Ow: 'Maaf, bot tidak dapat mengeluarkan owner *DABOT*!',
                Bk: 'Maaf, bot tidak dapat memblokir Owner *DABOT*!',
                Ad: 'Terjadi kesalahan, mungkin diprivasi!',
                Iv: 'Terjadi kesalahan, link tidak valid!'
            }
        }
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 SPAM STICKER TERDETEKSI 」*\nAnda terdeteksi spam stiker, anda dikick dari grup!', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh DABOT!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                tobz.reply(from, `Perintah sudah melebihi batas maksimal, coba lagi besok!`, id)
                                return true;
                            }else{
                                limit
                                found = true;
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '.mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nKetik .daftarpremium untuk menjadi member premium!', id)
                        ...
                        tobz.reply(from, 'Bot telah dinonaktifkan digrup ini, aktifkan kembali dengan perintah .unmute', id)
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                        ...
                        muted.push(chatId)
                        fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
                        reply(from, 'Bot telah dinonaktifkan digrup ini, aktifkan kembali dengan perintah .unmute', id)
                    }
                }
                if(body === '.unmute' && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        ...
                        fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
                        tobz.reply(from, 'Bot telah diaktifkan kembali digrup ini!', id)         
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                ...
                if (body === '.unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh owner *DABOT*!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh *DABOT*!', id)
        ...
        case prefix+'stiker':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
        ...
        case prefix+'ttp':
                ...
                if(cekumur(cekage)) return
                if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
                if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ttp <teks kamu>\nContoh : ${prefix}ttp Ambrose ganteng`, id)
                try
                {
        ...
        case prefix+'ttp2':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ttp2 <teks kamu>\nContoh : ${prefix}ttp2 AAmbrose ganteng`, id)
        ...
        case prefix+'ttg':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, `Perintah hanya dapat digunakan didalam grup!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            try {
                if (quotedMsgObj == null) {
                    if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ttg <teks kamu>\nContoh : ${prefix}ttg ambrose ganteng`, id)
        ...
        case prefix+'toimg':
            ...
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                tobz.reply(from, `Sedang diproses... Mohon untuk menunggu!`, id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await tobz.sendFile(from, imageBase64, 'imagesticker.jpg', 'Convert stiker ke gambar berhasil!', id)
            } else if (!quotedMsg) return tobz.reply(from, 'Reply sticker yang sudah dikirim!' , id)
            break
        ...
        case prefix+'lovemessage':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}lovemessage <teksnya>, contoh ${prefix}lovemessage Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const lovemsg = body.slice(12)
            if (lovemsg.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'romance':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}romance <teksnya>, contoh ${prefix}romance Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const rmnc = body.slice(9)
            if (rmnc.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'party':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}party <teksnya>, contoh ${prefix}party Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const prty = body.slice(7)
            if (prty.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'silk':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}silk <teksnya>, contoh ${prefix}silk Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const slkz = body.slice(5)
            if (slkz.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}blackpink <teksnya>, contoh ${prefix}blackpink Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const blpk = body.slice(11)
            if (blpk.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'thunder':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}thunder <teksnya>, contoh ${prefix}thunder Deni ambrose`, id)
            tobz.reply(from, mess.wait, id)
            const thndr = body.slice(9)
            if (thndr.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}pornhub /teks1 /teks2, contoh ${prefix}thunder /Deni /Dahyun`, id)
            argz = body.trim().split('/')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                const lpornhub = argz[1]
                const lpornhub2 = argz[2]
                if (lpornhub.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
                if (lpornhub2.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
                tobz.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${lpornhub}&text2=${lpornhub2}&apikey=${vhtearkey}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Kirim perintah ${prefix}pornhub /teks1 /teks2, contoh ${prefix}thunder /Deni /Dahyun`, id)
            }
            break
        case prefix+'googleteks':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}googleteks /teks1 /teks2 /teks3\nContoh ${prefix}googleteks /Dahyun /Dahyun cantik /Dahyun bini gw`, id)
            argz = body.trim().split('/')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                const google1 = argz[1]
                const google2 = argz[2]
                const google3 = argz[3]
                tobz.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${google1}&text2=${google2}&text3=${google3}&apikey=${vhtearkey}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Kirim perintah ${prefix}googleteks /teks1 /teks2 /teks3\nContoh ${prefix}googleteks /Dahyun /Dahyun cantik /Dahyun bini gw`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}glitch /teks1 /teks2, contoh ${prefix}glitch /Ambrose /Dahyun`, id)
            argz = body.trim().split('/')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                const glitch1 = argz[1]
                const glitch2 = argz[2]
                if (glitch1.length > 10) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
                if (glitch2.length > 15) return tobz.reply(from, 'Teks terlalu panjang, maksimal 10 huruf!', id)
                tobz.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${glitch1}&text2=${glitch2}&apikey=${vhtearkey}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Kirim perintah ${prefix}glitch /teks1 /teks2, contoh ${prefix}glitch /Ambrose /Dahyun`, id)
            }
            break
        case prefix+'daftar':
                argz = body.trim().split('/')
                ...
                const umurnye = argz[2]
                    if(isNaN(umurnye)) return await tobz.reply(from, 'Tidak dapat terverifikasi, umur harus berupa angka!', id)
                    if(umurnye >= 40) return await tobz.reply(from, 'Tidak dapat terverifikasi, umur terlalu tua maksimal 40!', id)
                    ...
                    var ceknya = nonye
                        ...
                        if (obj === true){
                            return tobz.reply(from, 'Anda sudah mendaftar, mohon untuk tidak spam pendaftaran atau anda akan diblock!', id) // BAKAL RESPON JIKA NO UDAH ADA
                        } else {
                            const mentah = await tobz.checkNumberStatus(nonye) // PENDAFTARAN
                            const msg = (`Pendaftaran berhasil\nSerial Number : ${SN}\nPada ${moment().format('DD/MM/YY HH:mm:ss')}
╔═══✪〘 DAFTAR 〙✪════════
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                            ...
                            if (!hasil) return tobz.reply(from, 'Nomor yang anda dikirimkan tidak valid, atau tidak terdaftar di Whatsapp!', id) 
                            {
                            const register = ({
                        ...
                        await tobz.reply(from, `Format salah, kirim ${prefix}daftar /nama/umur\nContoh : ${prefix}daftar /Ambrose/20`, id) //if user is not registered
                    }
                break
            case prefix+'daftarulang':
                    ...
                    if(umurnya >= 40) return await tobz.reply(from, 'Tidak dapat terverifikasi, umur terlalu tua maksimal 40!', id)
                        var found = false
                        ...
                        if (found !== false) {
                            ...
                            const result = (`Pendaftaran ulang berhasil\nSerial Number : ${SN}\nPada ${moment().format('DD/MM/YY HH:mm:ss')}
╔═══✪〘 DAFTAR ULANG 〙✪════════
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                            ...
                            tobz.sendTextWithMentions(from, result, id)
                                ...
                                tobz.reply(from, `${monospace(`Nomor belum pernah mendaftar!`)}`, id)
                        }
                break
        case prefix+'grupinfo' :
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', message.id)
            ...
            await tobz.sendFileFromUrl(from, pfp, 'group.png', `*「 INFORMASI GRUP 」*
*❖ Nama grup    : ${groupname}* 
*❖ Anggota grup : ${totalMem}*
*❖ fitur sambut : ${welgrp ? 'Aktif' : 'Tidak Aktif'}*
*❖ Fitur dewasa : ${ngrp ? 'Aktif' : 'Tidak Aktif'}*
*❖ Fitur spam stiker : ${stprt ? 'Aktif' : 'Tidak Aktif'}*
*❖ Fitur anti link : ${antlink ? 'Aktif' : 'Tidak Aktif'}*
*❖ Deskripsi grup* 
...
${desc}`)
        ...
        case prefix+'tts':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            try {
                if (args.length === 1) return tobz.reply(from, 'Kirim perintah !tts <kode bahasa> <teksnya>, Contoh .tts id deni ambrose\nUntuk melihat kode bahasa ketik .bahasa')
                var dataBhs = args[1]      
                const ttsHZ = require('node-gtts')(dataBhs)
                var dataText = body.slice(8)
                if (dataText === '') return tobz.reply(from, 'Teks tidak boleh kosong!', id)
                if (dataText.length > 500) return tobz.reply(from, 'Teks melebihi batas maksimal!', id)
        ...
        case prefix+'kapan':
            ...
            if (!when) tobz.reply(from, 'Format salah! ketik .kapankah <teksnya>, Contoh .kapankah saya nikah?')
        ...
        case prefix+'rate':
            ...
            if (!rating) tobz.reply(from, 'Format salah! ketik .nilai <teksnya>, Contoh .nilai kegantengan saya')
        ...
        case prefix+'apakah':
            ...
            if (!nanya) tobz.reply(from, 'Format salah! ketik .apakah <teksnya>, Contoh .apakah saya ganteng?')
         ...
         case prefix+'bisakah':
            ...
            if (!bsk) tobz.reply(from, 'Format salah! ketik !bisakah <teksnya>, Contoh .bisakah saya menjadi pintar?.')
            await tobz.sendText(from, `Pertanyaan: *${bsk}* \n\nJawaban: ${jbsk}`)
            break
        case prefix+'owner':
        case prefix+'creator':
        case prefix+'ownerbot':
            tobz.sendContact(chatId, `6285747852482@c.us`)
            tobz.reply(from, 'Itu nomor owner bot, chat ya!', id)
            break
        case prefix+'resetsticker':
        case prefix+'resetstiker':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, `Perintah hanya dapat digunakan oleh member *premium*!\nKetik ${prefix}daftarpremium untuk menjadi member premium!`, id)
            if (!args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}resetsticker @tagmember`, id) 
            const nomebr = args[1]
            ...
            const cuss = textz + '@c.us'
                ...
                if (found !== false) {
                    stickerspam[found].msg = 1;
                    const result = 'Stiker sudah direset!'
                    console.log(stickerspam[found])
                    fs.writeFileSync('./lib/database/stickerspam.json',JSON.stringify(stickerspam));
                    tobz.reply(from, result, from)
                    limitAdd(serial)
                } else {
                        tobz.reply(from, `Nomor tidak terdapat didalam database!`, id)
                }
            break
        ...
        case prefix+'antilink':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, `Perintah hanya dapat digunakan didalam grup!`, id)
            if (!isGroupAdmins) return tobz.reply(from, `Perintah hanya dapat digunakan oleh admin grup!`, id)
            if (!isBotGroupAdmins) return tobz.reply(from, `Perintah hanya dapat digunakan ketika bot menjadi admiN!`, id)
            if (args[1] == 'aktif') {
                var cek = antilink.includes(chatId);
                if(cek){
                    return tobz.reply(from, `*「 ANTI LINK GRUP 」*\nStatus : Sudah Aktif`, id) //if number already exists on database
                } else {
                    antilink.push(chatId)
                    fs.writeFileSync('./lib/database/antilink.json', JSON.stringify(antilink))
                    tobz.reply(from, `*「 ANTI LINK GRUP 」*\nStatus : Aktif`, id)
                }
                ...
                tobz.reply(from, `Pilih aktif atau nonaktif!`, id)
            }
            break    
        case prefix+'antispamsticker':
        case prefix+'antispamstiker':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, `Perintah hanya dapat digunakan didalam grup!`, id)
            if (!isGroupAdmins) return tobz.reply(from, `Perintah hanya dapat digunakan oleh admin grup!`, id)
        ...
        case prefix+'antibadword':
            ...
            if (args[1] == 'enable') {
                ...
                tobz.reply(from, `Pilih enable atau disable a!`, id)
            } 
            break   
        case prefix+'dewasa':
            if(isReg(obj)) return
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isGroupAdmins, !isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh admin grup!', id)
            ...
            if (args[1].toLowerCase() === 'aktif') {
                ...
                if(cek){
                    return tobz.reply(from, `Fitur dewasa sudah diaktifkan digrup ini\nGunakan ${prefix}dewasamenu untuk melihat list menu dewasa!`, id)
                } else {
                nsfw_.push(chat.id)
                fs.writeFileSync('./lib/database/nsfwz.json', JSON.stringify(nsfw_))
                tobz.reply(from, 'Fitur dewasa sudah diaktifkan digrup ini, gunakan .dewasamenu untuk melihat list menu dewasa!', id)
                }
            } else if (args[1].toLowerCase() === 'nonaktif') {
                var cek = nsfw_.includes(chatId);
                if(cek){
                    return tobz.reply(from, `Fitur dewasa dinonaktifkan digrup ini!`, id)
                } else {
                nsfw_.splice(chat.id, 1)
                fs.writeFileSync('./lib/database/nsfwz.json', JSON.stringify(nsfw_))
                tobz.reply(from, 'Fitur dewasa dinonaktifkan digrup ini!!', id)
                }
            } else {
                tobz.reply(from, 'Pilih aktif atau nonaktif!', id)
            }
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
        ...
        case prefix+'grup':
            ...
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah hanya dapat digunakan jika bot menjadi admin!', id)
            ...
            if (args[1].toLowerCase() === 'open') {
                ...
                tobz.sendTextWithMentions(from, `Grup telah dibuka oleh admin @${sender.id.replace('@c.us','')}\nSekarang *semua member* dapat mengirim pesan`)
            } else if (args[1].toLowerCase() === 'close') {
                tobz.setGroupToAdminsOnly(groupId, true)
                tobz.sendTextWithMentions(from, `Grup telah ditutup oleh admin @${sender.id.replace('@c.us','')}\nSekarang *hanya admin* yang dapat mengirim pesan`)
            } else {
                tobz.reply(from, 'Pilih open atau close!', id)
            }
            break
        case prefix+'left':
            if(isReg(obj)) return
        ...
        case prefix+'sambutmember':
            ...
            if (args[1].toLowerCase() === 'aktif') {
                ...
                tobz.reply(from, 'Fitur welcome berhasil diaktifkan digrup ini!', id)
            } else if (args[1].toLowerCase() === 'nonaktif') {
                welkom.splice(chat.id, 1)
                fs.writeFileSync('./lib/database/welcome.json', JSON.stringify(welkom))
                tobz.reply(from, 'Fitur welcome berhasil dinonaktifkan digrup ini!', id)
            } else {
                tobz.reply(from, 'Pilih aktif atau nonaktif', id)
            }
            break
        ...
        case prefix+'neonime':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}neonime <query>, Contoh : ${prefix}neonime naruto`)
        ...
        case prefix+'kusonime':
            ...
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah .kusonime <query>, Contoh : .kusonime naruto', id)
        ...
        case prefix+'dewabatch':
            ...
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah .dewabatch <query>, Contoh : .dewabatch naruto', id)
        ...
        case prefix+'pinterest':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah .pinterest <query>, Contoh : .pinterest Dahyun', id)
        ...
        case prefix+'nhview':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isNsfw) return tobz.reply(from, 'Perintah belum dapat digunakan, karena fitur belum diaktifkan oleh admin!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
        ...
        case prefix+'bkp': // MFARELS
            ...
            await limitAdd(serial)
            tobz.sendText(from, bkp())
            break
        // MFARELS
        case prefix+'animengewe':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            const trapnime = await axios.get('https://tobz-api.herokuapp.com/api/nsfwtrap?apikey=' + tobzkey)
            const trapnim = trapnime.data
            if (trapnim.result.endsWith('.png')) {
                var ext = '.png'
            } else {
                var ext = '.jpg'
            }
            tobz.sendImage(from, trapnim.result, `RandomTrapnime${ext}`, 'Random Trapnime!', id)
            await limitAdd(serial)
            break
        case prefix+'hentai':
            if(isReg(obj)) return
        ...
        case prefix+'nhder':
            ...
            if (args.length >=2){
                ...
                tobz.sendText(from, 'Masukan kode hentainya!')
            }
            break
        ...
        case prefix+'quotesnime':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            const mantap = await axios.get('https://tobz-api.herokuapp.com/api/quotesnime/random?apikey=' + tobzkey)
            mantap = mantap.data
            tobz.reply(from, `➸ *Quotes* : ${mantap.quote}\n➸ *Character* : ${mantap.character}\n➸ *Anime* : ${mantap.anime}`, id)
            await limitAdd(serial)
            break
        case prefix+'meme':
            if(isReg(obj)) return
        ...
        case prefix+'maluser':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            const usernamej = body.slice(18)
            tobz.reply(from, mess.wait, id)
            try {
                const result = await axios.get(`https://api.jikan.moe/v3/user/${usernamej}`)
                const jikan =  result.data
                const Data = `*「 USER - MYANIMELIST 」*

• Username: ${jikan.usernamej}
...
🌐️ *URL*: ${url}`
        ...
        case prefix+'quran':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}quran <urutan surah>\nContoh : ${prefix}quran 1`, id)
            ...
            await tobz.reply(from, `${hasqu}`, id).catch((e) => tobz.reply(from, `Mungkin urutan surah ke-${args[1]} tidak terdaftar dalam list surah!`, id))
            await limitAdd(serial)
            break
        ...
        case prefix+'infosurah': // ARUGAZ
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah !infosurah <nama surah>\nContoh : ${prefix}infosurah al-fatihah\nUntuk nama surah cek di ${prefix}listsurah`, message.id)
                ...
                try {
                    ...
                    tobz.reply(from, 'Nama surah tidak ditemukan, ketik !listsurah untuk melihat daftar surah. Penulisan koma, titik harus benar!', id)
                }
            break
        case prefix+'tafsir': // ARUGAZ
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah ${prefix}tafsir <nama surah> <ayat surah>\nContoh : ${prefix}tafsir al-fatihah 2`, message.id)
        ...
        case prefix+'ytsearch':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ytsearch <query>, Contoh : ${prefix}ytsearch Despacito`)
        ...
        case prefix+'shopee':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}shopee <nama barang>, Contoh : ${prefix}shopee Iphone 12 Pro max`)
        ...
        case prefix+'playstore':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}playstore <aplikasi>, Contoh : ${prefix}playstore tiktok`)
        ...
        case prefix+'newstickerline':
            ...
            break
        case prefix+'beritaterbaru':
        case prefix+'beritabaru':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'infogempa':
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            await limitAdd(serial)
            const sgempa = await axios.get('https://tobz-api.herokuapp.com/api/nsfwblowjob?apikey=' + tobzkey)
            const rgempa = sgempa.data
            tobz.reply(from, `➸ *Quotes* : ${sgempa.kedalaman}`, id)
            break
        case prefix+'ssphone':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *.ssphone [linkWeb]*\nContoh : *.ssphone http://www.instagram.com/deniambrose_*', id)
    ...
    case prefix+'bitiy':
        ...
        case prefix+'ytmp4':
            ...
            if (!isAdmin) return tobz.reply(from, `Perintah hanya dapat digunakan oleh member *premium*\nKetik .daftarpremium untuk menjadi member premium!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ytmp4 <link yt>\nContoh : ${prefix}ytmp4 https://www.youtube.com/watch?v=lqNLvPxUlnI`)
            ...
            try {
                ...
                const ytvh2 = await ytvh.json()
                 if (ytvh2.status == false) {
                    tobz.reply(from, `Terjadi kesalahan! kemungkinan link tidak valid.`, id)
                } else {
        ...
        case prefix+'play':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, `Perintah hanya dapat digunakan oleh member *premium*!`, id)
        ...
        case prefix+'ytmp3':
            ...
            if (!isAdmin) return tobz.reply(from, `Perintah hanya dapat digunakan oleh member *premium*\nKetik .daftarpremium untuk menjadi member premium!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}ytmp3 <link yt>\nContoh : ${prefix}ytmp3 https://www.youtube.com/watch?v=lqNLvPxUlnI`, id)
	...
	case prefix+'moddroid':
        ...
        case prefix+'google':
            ...
            if(googleQuery == undefined || googleQuery == ' ') return tobz.reply(from, `Kirim perintah ${prefix}google <query>, Contoh : ${prefix}google Kapan whatsaap didirikan?`, id)
            ...
            if(args.length >= 2){
                ...
                translatte(textai, {to: codelang}).then(res => {
                    ...
                    limitAdd(serial)
                     ...
                     tobz.sendText(from,`Kirim perintah ${prefix}translate <kode bahasa> <teksnya>\nContoh : ${prefix}translate id Where ur from?\NUntuk kode bahasa ketik ${prefix}bahasa`);
                });
            }
            break
        ...
        case prefix+'ramalpasangan':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'kirim perintah !ramalpasangan ambrose & dahyun', id)
            argz = body.trim().split('&')
            if (argz.length >= 2) {
            tobz.reply(from, mess.wait, id)
            const kamu = argz[0]
        ...
        case prefix+'artinama':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}artinama <nama>, Contoh : ${prefix}artinama Ambrose`, id)
            try {
        ...
        case prefix+'zodiak':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah .zodiaka <zodiak>, Contoh : .zodiak Scorpio', id)
            try {
        ...
        case prefix+'artimimpi':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah .artimimpi <mimpi apa>\nContoh : .artimimpi ular', id)
            try {
        ...
        case prefix+'wiki':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah .wiki <query>, Contoh : .wiki reinkarnasi`, id)
            const wiki1 = body.slice(6)
            const wiki2 = await axios.get(`https://api.vhtear.com/wikipedia?query=${wiki1}&apikey=${vhtearkey}`)
            if (wiki2.data.error) {
                tobz.reply(from, wiki2.data.error, id)
            } else {
                tobz.sendText(from, `*「 WIKIPEDIA 」*\n\n➸ *Query* : ${wiki1}\n\n➸ *Result* : ${wiki2.data.result.Info}`, id)
                await limitAdd(serial)
            }
            break
        case prefix+'kbbi':
        ...
        case prefix+'googleimage':
            ...
            if(!jum) return await tobz.reply(from, `Jumlah gambar diperlukan, contoh = .googleimage |Dahyun|3`, id)
...
➸ *Cara:* ${cara}`
        ...
        case prefix+'igstalk':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *.igstalk @username*\nContoh *.igstalk deniambrose_*', id)
            const insta1 = body.slice(6)
            const insta2 = await axios.get(`https://api.vhtear.com/igprofile?query=${insta1}&apikey=${vhtearkey}`)
            const { username, full_name, follow, follower, post_count, biography, is_private } = insta2.data.result
            const capt = `User Ditemukan!!\n\n*➸ Nama :* ${insta2.result.full_name}\n*➸ Username :* ${insta2.result.username}\n*➸ Followers :* ${insta2.result.follower}\n*➸ Mengikuti :* ${insta2.result.follow}\n*➸ Jumlah Post :* ${insta2.result.post_count}\n*➸ Private :* ${insta2.result.is_private}\n*➸ Bio :* ${insta2.result.biography}`
                tobz.sendTextWithMentions(from, capt, id)
                await limitAdd(serial)
            break
        case prefix+'':
            if(isReg(obj)) return
        ...
        case prefix+'maps':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *.maps [optional]*, Contoh : *.maps kudus*')
            argz = body.trim().split(' ')
            console.log(...argz[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'textmaker':
                ...
                await tobz.sendFile(from, ImageBase64, 'image.png','nih a...')
                await limitAdd(serial)
                } else {
                await tobz.reply(from, 'Wrong Format!', id)
                }
        ...
        case prefix+'quoteit':
            ...
            if (argz.length >= 4) {
                ...
                tobz.reply(from, 'Kirim perintah .quotemaker /teksnya/namalo/temanya\nContoh ; .quotemaker /Dahyun cantik/ambrose/Laut', id)
            }
            break
        ...
        case prefix+'anulis':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *.nulis [teks]*, contoh *.nulis Dahyun bininya ambrose*', id)
        ...
        case prefix+'anjing':
            ...
            tobz.sendFileFromUrl(from, kya, 'Dog.jpeg', 'nih a..')
            await limitAdd(serial)
            break
        case prefix+'qrcode':
            if(isReg(obj)) return
        ...
        case prefix+'gachacewek':
            ...
            tobz.sendFileFromUrl(from, pep, 'pptl.jpg', 'nih a, Follow ig : https://www.instagram.com/deniambrose_', message.id)
            await limitAdd(serial)
            break
        case prefix+'kucing':
            if(isReg(obj)) return
            ...
            tobz.sendFileFromUrl(from, 'http://placekitten.com/'+q3+'/'+q2, 'neko.png','Nih a...')
            await limitAdd(serial)
            break
        case prefix+'pokemon':
            if(isReg(obj)) return
            ...
            tobz.sendFileFromUrl(from, 'https://assets.pokemon.com/assets/cms2/img/pokedex/full/'+q7+'.png','Pokemon.png','nih a...')
            await limitAdd(serial)
            break
        case prefix+'quote':
        case prefix+'quotes':
        ...
        case prefix+'lirik':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length == 1) return tobz.reply(from, 'Kirim perintah *.lirik [optional]*, contoh *.lirik sugeng dalu*', id)
        ...
        case prefix+'chord':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *.chord [query]*, contoh *.chord sugeng dalu*', id)
        ...
        case prefix+'tampar':
            ...
            tobz.sendTextWithMentions(from, '@' + person + ' Menampol dengan keras a wkwk, kadang kalo nampol itu sayang ' + slap)
            limitAdd(serial)
            break
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner DABOT`, id)
                bctxt = body.slice(4)
                txtbc = `*「 DABOT BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner DABOT', id)
            const groupMek = await tobz.getGroupMembers(groupId)
            let heho = '╔═══✪〘 GRUP 〙✪════════\n'
            for (let i = 0; i < groupMem.length; i++) {
                heho += '╠❖'
                heho += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
            }
            heho += '╚═══✪〘 DABOT 〙✪═══════'
            await sleep(2000)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nUntuk mendaftar menjadi member premium gunakan perintah .daftarpremium', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nUntuk mendaftar menjadi member premium gunakan perintah .daftarpremium', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'kick':
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber, groupAdmins).includes(mentionedJidList[i])) return tobz.reply(from, mess.error.Ki, id)
                await tobz.removeParticipant(groupId, mentionedJidList[i])
            }
            break
        case prefix+'okeluarkanbot':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nUntuk mendaftar menjadi member premium gunakan perintah .daftarpremium', id)
            await tobz.sendText(from,'DABOT DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'keluarkanbot':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            await tobz.sendText(from,'Sayonara').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'ojadiadmin':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nUntuk mendaftar menjadi member premium gunakan perintah .daftarpremium', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah hanya dapat digunakan jika bot menjadi admin!', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah hanya dapat digunakan didalam grup!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan DABOT`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan DABOT`, id)
            try {
                ...
                tobz.sendTextWithMentions(from, `Sombong amat a...\n${list}`)
            } catch(err) {
                console.log(err)
        ...
        case prefix+'sswa':
            ...
            tobz.sendFile(from, sesPic, 'session.png', 'Nih bosku...', id)
            break
        case prefix+'dabotadmin':
            let admn = `This is list of DABOT Admin\nTotal : ${adminNumber.length}\n`
        ...
        case prefix+'limit':
            ...
            break
        case prefix+'kasihlimit': // Hanya Admin & Owner Elaina yang bisa gift Limit
            if (!isAdmin, !isOwner) return tobz.reply(from, `Perintah hanya dapat digunakan oleh member *premium*!`, id)
                    const nomerr = arg.split(' ')[0]
                    const jmla = arg.split(' ')[1]
                    if(!nomerr) return tobz.reply(from, `Kirim perintah ${prefix}kasihlimit <@tagmember> <jumlah> \n=> Contoh : ${prefix}kasihlimit @6285747852482 15`, id)
                    let texta = nomerr.replace(/[-\s+@c.us]/g,'')
                    const cusz = texta + '@c.us'
                    if(!jmla) return tobz.reply(from, `Kirim perintah ${prefix}kasihlimit <@tagmember> <jumlah> \n=> Contoh : ${prefix}kasihlimit @6285747852482 15`, id)
                    if(jmla > 20) return await tobz.reply(from, `Maximal  20!`, id)
                        ...
                        if (found !== false) {
                            ...
                            if(limit[found].limit <= 20) return tobz.reply(from, `Kuota limit masih penuh tidak dapat menambahkannya lagi!`, id)
                            if(limit[found].limit <= 0) { // JIKA LIMIT 0 MAKA BISA GIFT
                                return tobz.reply(from, `Kuota limit masih penuh tidak dapat menambahkannya lagi!`, id)
                            }else{
                            limit[found].limit -= jmla
                            const updated = limit[found]
                            const result = `Tambah limir sukses dengan\nSerial Number : ${SN}\nPada ${moment().format('DD/MM/YY HH:mm:ss')}
╔═══✪〘 TAMBAH LIMIT 〙✪════════
║❖ Nama  : @${updated.id.replace('@c.us','')}
...
╚═══✪〘 DABOT 〙✪═══════`
        ...
        case prefix+'addpremium':
            if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh *DABOT*!', id)
                ...
                tobz.reply(from, 'Sukses menambahkan sebagai *Member premium*!', id)
                }
            break
        case prefix+'delpremium':
            if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh *DABOT*!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Sukses menghapus dari *Member premium*!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh *DABOT*!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'unblock':
            ...
            break
        case prefix+'gantinama':
            if (!isOwner) return tobz.reply(from, `Perintah hanya dapat digunakan oleh *DABOT*!`, id)
                const setnem = body.slice(9)
                await tobz.setMyName(setnem)
                tobz.sendTextWithMentions(from, `Makasih Nama Barunya @${sender.id.replace('@c.us','')} a`)
            break
        case prefix+'gantistatus':
            if (!isOwner) return tobz.reply(from, `Perintah hanya dapat digunakan oleh *DABOT*!`, id)
                const setstat = body.slice(11)
                await tobz.setMyStatus(setstat)
                tobz.sendTextWithMentions(from, `Makasih Status Barunya @${sender.id.replace('@c.us','')} a`)
            break
        case prefix+'gantiprofil':
            if (!isOwner) return tobz.reply(from, `Perintah hanya dapat digunakan oleh *DABOT*!`, id)
            if (isMedia) {
                const mediaData = await decryptMedia(message)
                const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                await tobz.setProfilePic(imageBase64)
                tobz.sendTextWithMentions(`Makasih @${sender.id.replace('@c.us','')} Foto Profilenya a`)
            } else if (quotedMsg && quotedMsg.type == 'image') {
                const mediaData = await decryptMedia(quotedMsg)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await tobz.setProfilePic(imageBase64)
                tobz.sendTextWithMentions(from, `Makasih @${sender.id.replace('@c.us','')} Foto Profilenya a`)
            } else {
                tobz.reply(from, `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan ${prefix}setprofilepic`, id)
            }
            break
        case prefix+'colongfoto':
            if (!isGroupMsg) return tobz.reply(from, `Perintah hanya dapat digunakan didalam grup!`, id)
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin DABOT!`, id)
	...
	// Fadhlur Owner of NotBot Bug? wa : wa.me/6281395771492
	    ...
	    break
	//list menu
        case prefix+'waktubot':
            tobz.reply(from, `DABOT telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
            tobz.sendText(from, helpcmd(prefix, cts, pendaftar))
            break
        case prefix+'dagroup':
            tobz.reply(from, `Link Group DABOT : https://chat.whatsapp.com/DzRKpHZgXsK1416iXMGubM\nJangan Lupa Join Ya a ${pushname}`, id)
            break
        case prefix+'grupmenu':
            tobz.sendText(from, grupcmd(prefix))
            break
        case prefix+'funmenu':
        ...
        case prefix+'premiummenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nUntuk mendaftar menjadi member premium gunakan perintah .daftarpremium', id)
            tobz.sendText(from, premiumcmd(prefix))
            break
        ...
        case prefix+'islamimenu':
            tobz.sendText(from, islamcmd(prefix))
            break
        case prefix+'makermenu':
            tobz.sendText(from, editorcmd(prefix))
            break
        case prefix+'searchmenu':
            tobz.sendText(from, searchcmd(prefix))
            break
        case prefix+'editormenu':
            tobz.sendText(from, makercmd())
            break
        case prefix+'dewasamenu':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah hanya dapat digunakan didalam grup!', id)
            if (!isNsfw) return tobz.reply(from, 'Perintah belum dapat digunakan, karena fitur belum diaktifkan oleh admin!', id)
            tobz.sendText(from, dewasacmd(prefix))
            break
        // INFORMATION
        case prefix+'donasi':
...
//BY SYAHRUL SARIZ
        case prefix+'nulis':
            await tobz.reply(from, `Perintah nulis terdapat dua versi\nVersi 1 Menggunakan Nama,kelas dan tanggal\nUntuk menggunakannya ketik ${prefix}nulis1\n\nVersi 2 Hanya tulisan saja\nUntuk menggunakannya ketik ${prefix}nulis2`, id)
            break
        case prefix+'nulis1':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, 'Perintah hanya dapat digunakan oleh member *premium*!\nKetik !daftarpremium untuk menjadi member premium!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}nulis1 /nama/kelas/teksnya, contoh ${prefix}nulis1 /Deniambrose/13TKR2/tulisanya a`, id)
            argz = body.trim().split('/')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                const namanyee = argz[1]
                const kelasnyee = argz[2]
                const teksnyee = argz[3]
                tobz.sendFileFromUrl(from, `https://api.zeks.xyz/api/magernulis?nama=${namanyee}&kelas=${kelasnyee}&text=${teksnyee}&tinta=4`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Kirim perintah ${prefix}nulis1 /nama/kelas/teksnya\nContoh : \n${prefix}nulis1 /syahrul/12akuntansi/Ujian tengah semester`, id)
            }
            break
        case prefix+'nulis2':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf kuota limit kamu sudah habis, hubungi owner untuk mendapatkan limit lagi!\nNomor owner dapat didapatkan dengan mengetik ${prefix}owner`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah ${prefix}nulis2 <teksnya>\nContoh : \n${prefix}nulis2 ujian tengah semester 2`, id)
            tobz.reply(from, mess.wait, id)
            const nulis2 = body.slice(9)
            await tobz.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${nulis2}&apikey=${vhtearkey}`, '', id)
            break
        default:
            ...
            if (command.startsWith('.')) {
                tobz.reply(from, `Maaf a ${pushname}, Perintah *${args[0]}* tidak terdaftar didalam ${prefix}menu`, id)
            }
            await tobz.sendSeen(from) 
            }
        }
