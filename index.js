...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        lolcatjs.fromString(color(figlet.textSync('DABOT', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        lolcatjs.fromString('[DEV] DABOT')
        lolcatjs.fromString('[SERVER] SERVER READY!')
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, 'Hallo, Saya *DABOT* gunakan .menu untuk melihat menu bot!')
            if(mtcState === false){
                const groups = await tobz.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await tobz.sendText(chat.id, 'Maaf, batas yang *DABOT* tampung sudah melebihi batas maksimal!').then(async () =>{
                        tobz.deleteChat(chat.id)
                        tobz.leaveGroup(chat.id)
                    })
                }else{
                    if(chat.groupMetadata.participants.length < memberLimit){
                        await tobz.sendText(chat.id, `Maaf! Untuk *DABOT* keluar jika member grup tidak melebihi ${memberLimit} orang`).then(async () =>{
                            tobz.deleteChat(chat.id)
                            tobz.leaveGroup(chat.id)
                        })
                    }else{
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Hallo, Saya *DABOT* gunakan !menu untuk melihat menu bot!.')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'DABOT sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
        ...
        tobz.onIncomingCall(( async (call) => {
            await tobz.sendText(call.peerJid, 'Telpon terdeteksi a, BOT memiliki sistem block otomatis jika ditelpon\nMaaf anda diblock oleh bot, Chat owner untuk membuka block\nOwner number : https://wa.me/6285747852482')
